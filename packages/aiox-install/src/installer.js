...
async function runDoctor(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npx aiox-antigravity doctor');
    return;
  }
...
async function initializeAiox(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npx aiox-antigravity install');
    return;
  }
...
async function runInstaller(options = {}) {
  ...
  if (brownfield.isBrownfield) {
    ...
    if (brownfield.hasLegacyConfig && !brownfield.hasLayeredConfig) {
      ...
      if (shouldMigrate && !options.dryRun) {
        ...
        logger.action('Run: npx aiox-antigravity config migrate');
      }
    } else {
  ...
  if (options.dryRun) {
    ...
    console.log(chalk.dim('  1. Run `npx aiox-antigravity info` to see your configuration'));
