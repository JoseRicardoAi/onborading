...
class AIOXUpdater {
    ...
    try {
      ...
      if (!result.latest) {
        ...
        if (!isOnline) {
          ...
          result.error = 'Package aiox-antigravity not found on npm registry. This may be a local development installation.';
        }
        return result;
      }
  ...
  async getInstalledVersion() {
    ...
    const packageJsonPath = path.join(this.projectRoot, 'node_modules', 'aiox-antigravity', 'package.json');
    ...
    if (fs.existsSync(localPackageJsonPath)) {
      try {
        ...
        if (pkg.name === '@evoluindocomia/aiox-antigravity' || pkg.name === 'aiox-antigravity') {
  ...
  async getLatestVersion() {
    return new Promise((resolve) => {
      const request = https.get(
        'https://registry.npmjs.org/aiox-antigravity/latest',
  ...
  async applyUpdate(targetVersion, _customizedFiles = []) {
    ...
    try {
      ...
      const cmd = `npm install aiox-antigravity@${targetVersion} --save-dev`;
      ...
      this.log('Synchronizing structural directories...');
      const { 
        copySkillFiles, 
        ...
        copyApplicationSquads
      } = require('../wizard/ide-config-generator');
      
      this.log('Copying skills...');
      await copySkillFiles(this.projectRoot);
      
      this.log('Copying workflows...');
      await copyExtraCommandFiles(this.projectRoot);
      
      this.log('Copying docs...');
      await copyAntigravityDocs(this.projectRoot);

      this.log('Copying agents...');
      const { getIDEConfig } = require('../config/ide-configs');
      const ideConfig = getIDEConfig('antigravity');
      if (ideConfig) {
        try {
          await copyAgentFiles(this.projectRoot, ideConfig.agentFolder, ideConfig);
        } catch (e) {
          this.log(`Agents copy warning: ${e.message}`);
        }
      }

      this.log('Copying Gemini hooks...');
      try {
        await copyGeminiHooksFolder(this.projectRoot);
      } catch (e) {
        this.log(`Gemini hooks copy warning: ${e.message}`);
      }

      this.log('Copying squads...');
      try {
        await copyApplicationSquads(this.projectRoot);
      } catch (e) {
        this.log(`Squads copy warning: ${e.message}`);
      }

      this.log('Running IDE Sync...');
      // Note: path resolution needs to hit .aiox-core at the project root
      const syncPath = path.join(this.projectRoot, '.aiox-core', 'infrastructure', 'scripts', 'ide-sync', 'index.js');
      if (fs.existsSync(syncPath)) {
        const { commandSync } = require(syncPath);
        const savedCwd = process.cwd();
        try {
          process.chdir(this.projectRoot);
          await commandSync({ quiet: true });
        } catch (e) {
          this.log(`IDE Sync warning: ${e.message}`);
        } finally {
...
function formatCheckResult(result, options = {}) {
  ...
  switch (result.status) {
    ...
    case UpdateStatus.UPDATE_AVAILABLE:
      ...
      lines.push(`  Run ${c.cyan}npx aiox-antigravity update${c.reset} to update.`);
      break;
...
function formatUpdateResult(result, options = {}) {
  ...
  if (result.success) {
    ...
    lines.push('');
    lines.push(`Run ${c.cyan}npx aiox-antigravity validate${c.reset} to verify installation.`);
  } else {
