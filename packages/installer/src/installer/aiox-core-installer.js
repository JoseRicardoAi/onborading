...
const FOLDERS_TO_COPY = [
  ...
  'monitor', // Antigravity Code hooks for monitoring
...
/**
 * Project-root operational artifacts required by the runtime-unico model.
 * These live outside .aiox-core and are copied conservatively for brownfield projects.
 * @constant {{name: string, required: boolean}[]}
 */
const OPERATIONAL_ROOTS_TO_COPY = [
  { name: '.antigravity', required: true },
  { name: '.codex', required: true },
  { name: '.synapse', required: true },
  { name: 'squads', required: true },
];
...
async function generateVersionJson(options) {
  const { targetAioxCore, version, installedFiles, mode = 'project-development' } = options;
...
/**
 * Recursively list files under a directory relative to the given base path.
 * @param {string} dir - Directory to scan
 * @param {string} baseDir - Base directory used for relative paths
...
/**
 * Copy project-root operational artifacts required by supported runtimes.
 * Existing files are preserved to avoid destructive brownfield installs.
 *
 * @param {string} packageRoot - Package root containing operational artifacts
 * @param {string} targetDir - Target project directory
 * @param {Object} result - Installation result object
 * @returns {Promise<void>}
 */
async function copyOperationalRootArtifacts(packageRoot, targetDir, result) {
  result.installedOperationalRoots = result.installedOperationalRoots || [];
  result.operationalRootFiles = result.operationalRootFiles || [];

  for (const root of OPERATIONAL_ROOTS_TO_COPY) {
    const source = path.join(packageRoot, root.name);
    const dest = path.join(targetDir, root.name);

    if (!(await fs.pathExists(source))) {
      if (root.required) {
        result.errors.push(`Operational root missing in package: ${root.name}`);
      }
      continue;
    }

    await fs.copy(source, dest, { overwrite: false, errorOnExist: false });
    result.installedFolders.push(root.name);
    result.installedOperationalRoots.push(root.name);

    const rootFiles = await listRelativeFiles(source, source);
    result.operationalRootFiles.push(
      ...rootFiles.map((file) => path.join(root.name, file).replace(/\\/g, '/')),
    );
  }
}
...
async function installAioxCore(options = {}) {
  ...
  try {
    ...
    // Runtime unico: install project-root artifacts for all supported runtimes.
    spinner.text = 'Copying operational runtime artifacts...';
    try {
      const packageRoot = path.join(__dirname, '..', '..', '..', '..');
      await copyOperationalRootArtifacts(packageRoot, targetDir, result);
    } catch (err) {
      spinner.warn(`Could not completely copy operational runtime artifacts: ${err.message}`);
    }
    ...
    const manifest = {
      ...
      operational_roots: result.installedOperationalRoots,
      operational_files: result.operationalRootFiles,
    };
...
module.exports = {
  ...
  copyOperationalRootArtifacts,
  listRelativeFiles,
  generateVersionJson,
  generateFileHashes,
  FOLDERS_TO_COPY,
  ROOT_FILES_TO_COPY,
  OPERATIONAL_ROOTS_TO_COPY,
};
