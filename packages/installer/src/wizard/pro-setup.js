 ...
 * 2. @aiox-antigravity/pro package (brownfield: node_modules/@aiox-antigravity/pro/license/{name})
 * 3. Absolute path via aiox-core in node_modules (brownfield upgrade)
 * 4. Absolute path via @aiox-antigravity/pro in user project (npx context)
 *
 * Path 4 is critical for npx execution: when running `npx aiox-antigravity install`,
 * require() resolves from the npx temp directory, not process.cwd(). After
 * bootstrap installs @aiox-antigravity/pro in the user's project, only an
 * absolute path to process.cwd()/node_modules/@aiox-antigravity/pro/... works.
 *
...
function loadProModule(moduleName) {
  ...
  // 2. @aiox-antigravity/pro package (works when aiox-core is a local dependency)
  try {
    return require(`@aiox-antigravity/pro/license/${moduleName}`);
  } catch { /* not available */ }
  ...
  // 4. @aiox-antigravity/pro in user project (npx context — require resolves from
  //    temp dir, so we need absolute path to where bootstrap installed the package)
  try {
    const absPath = path.join(process.cwd(), 'node_modules', '@aiox-antigravity', 'pro', 'license', moduleName);
    return require(absPath);
  } catch { /* not available */ }
...
async function stepLicenseGateWithEmail() {
  ...
  if (!checkResult.isBuyer) {
    ...
    showInfo('  Issues: https://github.com/evoluindocomia/aiox-antigravity/issues');
    ...
    try {
      ...
      if (loginError.code === 'EMAIL_NOT_VERIFIED') {
        ...
        if (remaining > 0) {
          ...
          showInfo('Forgot your password? Visit https://aiox-license-server.vercel.app/reset-password');
          showInfo('Or open an issue: https://github.com/evoluindocomia/aiox-antigravity/issues');
...
async function stepInstallScaffold(targetDir, options = {}) {
  ...
  const proSourceDir = path.join(targetDir, 'node_modules', '@aiox-antigravity', 'pro');
  ...
  // Step 2b: Install @aiox-antigravity/pro if not present
...
async function runProWizard(options = {}) {
  ...
  // install @aiox-antigravity/pro first to get the license API, then proceed with gate.
  if (!loadLicenseApi()) {
    const fs = require('fs');
    const path = require('path');
    const { execSync } = require('child_process');
    ...
    // Install @aiox-antigravity/pro to get license module
    const proDir = path.join(targetDir, 'node_modules', '@aiox-antigravity', 'pro');
    ...
    Object.keys(require.cache).forEach((key) => {
      if (key.includes('license-api') || key.includes('@aiox-antigravity')) {
