...
const { generateIDEConfigs, showSuccessSummary, copySkillFiles, copyExtraCommandFiles, copyAntigravityDocs } = require('./ide-config-generator');
const {
...
/**
 * Map wizard language code to Antigravity Code settings.json language name (Story ACT-12)
 * Antigravity Code uses full language names, not ISO codes.
 */
const LANGUAGE_MAP = {
  en: 'english',
  pt: 'portuguese',
...
/**
 * Write language preference to Antigravity config (Story ACT-12)
 * Ensures user's selected language persists in IDE commands
 *
 * @param {string} language - Language code from wizard (en|pt|es)
 * @param {string} [projectDir] - Target directory (default: process.cwd())
 * @returns {Promise<boolean>} True if successful
 */
async function writeAntigravitySettings(language, projectDir = process.cwd()) {
  const antigravityDir = path.join(projectDir, '.antigravity');
  const settingsPath = path.join(antigravityDir, 'antigravity.json');

  try {
    await fse.ensureDir(antigravityDir);
...
/**
 * Get existing language from Antigravity config (Story ACT-12 - Idempotency)
...
async function getExistingLanguage(projectDir = process.cwd()) {
  const settingsPath = path.join(projectDir, '.antigravity', 'antigravity.json');
  ...
  try {
    if (await fse.pathExists(settingsPath)) {
      ...
      if (settings && settings.language) {
        return String(settings.language).toLowerCase().trim();
      }
    }
  } catch {
...
async function runWizard(options = {}) {
  try {
    ...
    if (options.quiet) {
      ...
      // Story ACT-12: Language delegated to Antigravity Code settings.json
      ...
      // Story ACT-12: Check idempotency via Antigravity Code settings.json
      let languageAnswer;
    ...
    // Story INS-4.3: AGP Configuration
    console.log('\n🔒 Configuring Antigravity Governance Pipeline (AGP)...');
    try {
      console.log(`✅ AGP: configured via governance-config.md`);
      answers.settingsGenerated = true;
      answers.settingsDenyCount = 0;
    } catch (error) {
      console.warn(`⚠️  AGP configuration failed: ${error.message}`);
    ...
    // New: Copy Antigravity documentation
    console.log('\n📖 Copying Antigravity documentation...');
    try {
      const docsResult = await copyAntigravityDocs(process.cwd());
      if (docsResult.skipped) {
        console.log('   ℹ️  Documentation: source not found (skipped)');
      } else {
        console.log(`✅ Documentation: ${docsResult.count} files copied to .aiox-antigravity-docs/`);
        
        // Add to .gitignore if it exists
        const gitignorePath = path.join(process.cwd(), '.gitignore');
        if (await fse.pathExists(gitignorePath)) {
          let gitignore = await fse.readFile(gitignorePath, 'utf8');
          if (!gitignore.includes('.aiox-antigravity-docs/')) {
            gitignore += '\n# AIOX Antigravity Documentation\n.aiox-antigravity-docs/\n';
            await fse.writeFile(gitignorePath, gitignore);
            console.log('✅ Added .aiox-antigravity-docs/ to .gitignore');
          }
        }
      }
      answers.docsCopied = docsResult.count;
    } catch (error) {
      console.warn(`⚠️  Documentation copy failed: ${error.message}`);
      answers.docsCopied = 0;
    }
    ...
    try {
      ...
      // Story ACT-12: Write language to Antigravity antigravity.json
      if (answers.language) {
        const langWritten = await writeAntigravitySettings(answers.language);
        if (langWritten) {
          console.log('  - Language written to .antigravity/antigravity.json');
        } else {
          console.warn('  - Failed to write language to .antigravity/antigravity.json');
        }
      }
    ...
    // Runtime unico: repeat IDE sync after core-config.yaml is generated.
    console.log('\nRunning IDE sync with final runtime selection...');
    {
      const targetProjectRoot = process.cwd();
      const savedCwd = process.cwd();
      try {
        ...
        await commandSync({ quiet: true });
        answers.ideSyncStatus = 'synced';
        console.log('IDE sync: synced with final runtime selection');
        ...
        if (answers.ideSyncValidation === 'drift') {
          console.warn('IDE sync validation: drift detected - run \'aiox doctor --fix\' post-install');
        }
      } catch (syncError) {
        console.warn(`IDE sync failed: ${syncError.message} - run 'aiox doctor --fix' post-install`);
        answers.ideSyncStatus = 'failed';
        answers.ideSyncValidation = 'skipped';
      } finally {
        process.chdir(savedCwd);
    ...
    try {
      ...
      if (isLLMRoutingInstalled()) {
        ...
        if (llmResult.success) {
          ...
          console.log('   • antigravity-max  → Uses Antigravity Max subscription');
          console.log('   • antigravity-free → Uses DeepSeek (~$0.14/M tokens)');
          console.log('\n   💡 For antigravity-free, add DEEPSEEK_API_KEY to your .env');
          answers.llmRoutingInstalled = true;
