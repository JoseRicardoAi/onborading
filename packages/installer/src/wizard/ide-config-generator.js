  ...
  try {
    for (const ideKey of selectedIDEs) {
      ...
      try {
        ...
        // Gemini parity with Antigravity Code: copy hooks and configure settings
        if (ideKey === 'gemini') {
...
/**
 * Copy .antigravity/skills/ directories during installation (Story INS-4.3, Gap #11)
 * @param {string} projectRoot - Project root directory
...
async function copySkillFiles(projectRoot, _sourceRoot) {
  const sourceDir = _sourceRoot
    ? path.join(_sourceRoot, '.antigravity', 'skills')
    : path.join(__dirname, '..', '..', '..', '..', '.antigravity', 'skills');
  const targetDir = path.join(projectRoot, '.antigravity', 'skills');
...
/**
 * Copy extra .antigravity/commands/ files during installation (Story INS-4.3, Gap #12)
...
async function copyExtraCommandFiles(projectRoot, _sourceRoot) {
  const sourceDir = _sourceRoot
    ? path.join(_sourceRoot, '.antigravity', 'workflows')
    : path.join(__dirname, '..', '..', '..', '..', '.antigravity', 'workflows');
  const targetDir = path.join(projectRoot, '.antigravity', 'workflows');
...
/**
 * Copy Antigravity documentation to the user project during installation
 * @param {string} projectRoot - Project root directory
 * @param {string} [_sourceRoot] - Override source root for testing (default: __dirname-relative)
 * @returns {Promise<{count: number, skipped: boolean}>} Copy result
 */
async function copyAntigravityDocs(projectRoot, _sourceRoot) {
  const sourceDir = _sourceRoot
    ? path.join(_sourceRoot, 'docs', 'pt', 'antigravity')
    : path.join(__dirname, '..', '..', '..', '..', 'docs', 'pt', 'antigravity');
  const targetDir = path.join(projectRoot, '.aiox-antigravity-docs');
  ...
  const walk = async (dir) => {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        count += await walk(path.join(dir, entry.name));
      } else {
        count++;
      }
    }
  ...
  const count = await walk(targetDir);
  return { count, skipped: false };
}

/**
 * Copy application squads during installation/update.
 * Explicitly preserves custom user squads by only syncing directories
 * that exist in the framework source.
 * @param {string} projectRoot - Project root directory
 * @param {string} [_sourceRoot] - Override source root for testing
 * @returns {Promise<{count: number, skipped: boolean}>} Copy result
 */
async function copyApplicationSquads(projectRoot, _sourceRoot) {
  const sourceDir = _sourceRoot
    ? path.join(_sourceRoot, 'squads')
    : path.join(__dirname, '..', '..', '..', '..', 'squads');
  const targetDir = path.join(projectRoot, 'squads');
  ...
  const entries = await fs.readdir(sourceDir, { withFileTypes: true });
  const squadDirs = entries.filter(d => d.isDirectory());
  let count = 0;
  ...
  for (const dir of squadDirs) {
    ...
    // For framework squads, we overwrite to ensure the latest version
    // Custom squads existing only in targetDir are naturally untouched
    await fs.copy(sourcePath, targetPath, { overwrite: true });
    count++;
  }
