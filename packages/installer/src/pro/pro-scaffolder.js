 ...
 * node_modules/@aiox-antigravity/pro/ into the user's project after
 * license activation.
 *
 ...
 * @param {string} proSourceDir - Path to pro package content (node_modules/@aiox-antigravity/pro)
...
async function scaffoldProContent(targetDir, proSourceDir, options = {}) {
  ...
  if (!await fs.pathExists(proSourceDir)) {
    result.errors.push(
      `Pro package not found at ${proSourceDir}. Run "npm install @aiox-antigravity/pro" first.`
    );
    return result;
  }
...
async function installSquadCommands(targetDir) {
  ...
  const ideTargets = [
    { check: path.join('.antigravity', 'workflows'), dest: (squad) => path.join('.antigravity', 'workflows', squad) },
