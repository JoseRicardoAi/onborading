...
function validateCoreConfigStructure(config) {
  ...
  if (config.ide) {
    ...
    } else if (config.ide.selected.length !== 1) {
      errors.push('ide.selected must contain exactly one active IDE/runtime');
    }

    if (config.ide.configs && typeof config.ide.configs === 'object') {
      const activeConfigs = Object.entries(config.ide.configs)
        .filter(([, enabled]) => enabled === true)
        .map(([ide]) => ide);
      if (activeConfigs.length !== 1) {
        errors.push('ide.configs must enable exactly one IDE/runtime');
      }
      if (
        config.ide.configs.codex === true
        && (config.ide.configs.antigravity === true || config.ide.configs['antigravity-code'] === true)
      ) {
