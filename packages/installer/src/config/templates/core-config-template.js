...
function generateCoreConfig(options = {}) {
  ...
  const activeIDE = selectedIDEs.length > 0 ? selectedIDEs[0] : 'codex';
  ...
  const config = {
    ...
    // Language: Delegated to Antigravity native antigravity.json (Story ACT-12)
    // See: ~/.antigravity/antigravity.json → { "language": "portuguese" }

    // IDE Configuration (from Story 1.4)
    ide: {
      selected: [activeIDE],
      configs: {
        vscode: activeIDE === 'vscode',
        codex: activeIDE === 'codex',
        gemini: activeIDE === 'gemini',
        cursor: activeIDE === 'cursor',
        'github-copilot': activeIDE === 'github-copilot',
        antigravity: activeIDE === 'antigravity',
        zed: activeIDE === 'zed',
        'antigravity-desktop': activeIDE === 'antigravity-desktop',
        'antigravity-code': activeIDE === 'antigravity-code',
      },
    },
    ...
    ideSync: {
      ...
      targets: {
        'antigravity-code': {
          enabled: activeIDE === 'antigravity-code',
          path: '.antigravity/commands/AIOX/agents',
          format: 'full-markdown-yaml',
        },
        codex: {
          enabled: activeIDE === 'codex',
          path: '.codex/agents',
          format: 'full-markdown-yaml',
        },
      },
    ...
    mcp: {
      ...
      configLocation: '.antigravity/mcp.json',
