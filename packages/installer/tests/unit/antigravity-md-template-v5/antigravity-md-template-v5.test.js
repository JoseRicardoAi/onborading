...
const TEMPLATE_PATH = path.join(
  __dirname, '..', '..', '..', '..', '..', '.aiox-core',
  'product', 'templates', 'ide-rules', 'antigravity-rules.md'
);

describe('ANTIGRAVITY.md Template v5 (Story INS-4.4)', () => {
  let templateContent;
  ...
  describe('upgrade: new sections added to existing ANTIGRAVITY.md', () => {
    let merger;
    ...
    test('upgrade adds 4 new sections to ANTIGRAVITY.md without them', async () => {
      const existingAntigravityMd = `# AIOX Antigravity
...
User content that must be preserved`;
      ...
      const result = await merger.merge(existingAntigravityMd, templateContent);
    ...
    test('upgrade preserves custom PROJECT-CUSTOMIZED content', async () => {
      const existingAntigravityMd = `# My Project Rules
