...
function createTempProject(boundary, existingSettings) {
  ...
  // Create .antigravity directory and optionally existing settings
  const antigravityDir = path.join(tmpDir, '.antigravity');
  fs.mkdirSync(antigravityDir, { recursive: true });

  if (existingSettings) {
    fs.writeFileSync(
      path.join(antigravityDir, 'antigravity.json'),
  ...
  describe('idempotency', () => {
    test('running generator twice produces identical output', () => {
      ...
      try {
        // First run
        generate(tmpDir);
        const firstRun = fs.readFileSync(path.join(tmpDir, '.antigravity', 'antigravity.json'), 'utf8');

        // Second run
        generate(tmpDir);
        const secondRun = fs.readFileSync(path.join(tmpDir, '.antigravity', 'antigravity.json'), 'utf8');
    ...
    test('JSON output is valid and parseable', () => {
      ...
      try {
        generate(tmpDir);
        const content = fs.readFileSync(path.join(tmpDir, '.antigravity', 'antigravity.json'), 'utf8');
