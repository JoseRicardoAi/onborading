...
Improve tool selection accuracy by providing concrete input examples for the most-used tools within the Antigravity framework. When choosing which tool to use, reference these examples to match the current task to the correct tool.

## ADR-5 Compliance

These examples apply ONLY to the primary tools available in the Antigravity ecosystem. Avoid using shell workarounds (like `cat`, `grep`, `ls`) via `run_command` when a specific tool is available.

## Tool Examples

### view_file — Ler conteúdo de arquivo
...
### replace_file_content / multi_replace_file_content — Editar código
Use sempre estas ferramentas no lugar de scripts `sed` ou reescrever arquivos manualmente via shell.
- **Edição simples (substituir bloco único):** Use `replace_file_content` fornecendo o trecho exato de `TargetContent` e o `ReplacementContent`.
- **Múltiplas edições:** Use `multi_replace_file_content` com um array de `ReplacementChunks`.
...
### search_web — Buscar informações contextuais recentes
...
### mcp_stitch_generate_screen_from_text — Integração Stitch (UI Builder)
Use para gerar componentes visuais rapidamente. Parte do arsenal Antigravity.
- **Gerar Mockup:** `prompt: "Create a modern login screen with email and password fields"`, `projectId: "..."`

## Reference

Tool usage should strictly adhere to `tool-usage.md`. Never fallback to generic scripting (bash/powershell) for file modifications when structural tools (`replace_file_content`) exist.
