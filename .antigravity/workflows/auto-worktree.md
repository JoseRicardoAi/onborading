...
> [!NOTE]
> No AntiGravity, worktrees são gerenciados via `run_command` manualmente. O Antigravity CLI usava hooks automáticos que não existem aqui. A responsabilidade de criar/limpar worktrees é instrucional.

---

## Step 0: Verificação do Projeto

1. Confirmar que o AIOX está instalado no projeto (`.antigravity/` e `.aiox-core/` presentes na raiz).
2. NUNCA criar worktrees baseados em diretórios de configuração do framework (`.antigravity/`, `.aiox-core/`).
...
## Adaptação AntiGravity vs Antigravity CLI

| Funcionalidade     | Antigravity CLI        | AntiGravity          |
| ------------------ | ------------------ | -------------------- |
