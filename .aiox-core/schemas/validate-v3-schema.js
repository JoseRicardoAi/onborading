 ...
 *   --strict    Require V3 autoAntigravity section (fail on V2-only files)
...
/**
 * Check if file has V3 autoAntigravity section
 */
...
function validateRequiredFields(parsed, schema, type) {
  ...
  if (type === 'agent') {
    ...
    // V3 specific checks
    if (!parsed.autoAntigravity) {
      warnings.push('No autoAntigravity section (V2 format)');
    }
  }
...
function getV2V3Diff(parsed, type) {
  ...
  if (type === 'agent') {
    ...
    // V3 fields (new)
    if (parsed.autoAntigravity) {
      v3Fields.push('autoAntigravity.version');
      if (parsed.autoAntigravity.specPipeline) v3Fields.push('autoAntigravity.specPipeline');
      if (parsed.autoAntigravity.execution) v3Fields.push('autoAntigravity.execution');
      if (parsed.autoAntigravity.recovery) v3Fields.push('autoAntigravity.recovery');
      if (parsed.autoAntigravity.qa) v3Fields.push('autoAntigravity.qa');
      if (parsed.autoAntigravity.memory) v3Fields.push('autoAntigravity.memory');
      if (parsed.autoAntigravity.worktree) v3Fields.push('autoAntigravity.worktree');
    }
  }
  ...
  if (type === 'task') {
    ...
    if (parsed.autoAntigravity) {
      v3Fields.push('autoAntigravity.version');
      if (parsed.autoAntigravity.deterministic !== undefined) v3Fields.push('autoAntigravity.deterministic');
      if (parsed.autoAntigravity.elicit !== undefined) v3Fields.push('autoAntigravity.elicit');
      if (parsed.autoAntigravity.pipelinePhase) v3Fields.push('autoAntigravity.pipelinePhase');
      if (parsed.autoAntigravity.verification) v3Fields.push('autoAntigravity.verification');
      if (parsed.autoAntigravity.selfCritique) v3Fields.push('autoAntigravity.selfCritique');
      if (parsed.autoAntigravity.recovery) v3Fields.push('autoAntigravity.recovery');
    }
  }
...
async function validateFile(filePath, options = {}) {
  ...
  try {
    ...
    if (strict && version === 'V2') {
      errors.push('Strict mode: File is V2 format, missing autoAntigravity section');
    }
...
Options:
  ...
  --strict             Require V3 autoAntigravity section
  --json               Output as JSON
    `);
    return;
  }
