...
class CodeGraphProvider extends CodeIntelProvider {
  ...
  async _callMcpTool(toolName, params = {}) {
    // MCP tool calls are executed via the Antigravity Code MCP protocol.
    // In production, this is invoked by the Antigravity Code runtime.
    // For testing, this method is mocked.
    //
    // The actual MCP call signature:
    //   mcp__<server>__<tool>(params)
    //
    // Since we run inside Antigravity Code agent context, the MCP call
