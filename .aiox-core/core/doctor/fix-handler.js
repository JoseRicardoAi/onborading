...
const FIX_MAP = {
  'rules-files': {
    describe(result, { projectRoot, frameworkRoot }) {
      const rulesSource = path.join(frameworkRoot, '.antigravity', 'rules');
      const rulesTarget = path.join(projectRoot, '.antigravity', 'rules');
  ...
  'antigravity-md': {
    describe() {
      return 'Regenerate ANTIGRAVITY.md with missing sections';
    },
    async apply() {
      return 'ANTIGRAVITY.md regeneration requires npx aiox-core install --force';
    },
  },
