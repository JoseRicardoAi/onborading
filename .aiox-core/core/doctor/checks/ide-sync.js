 ...
 *
 * Validates agents in .antigravity/agents/ match
...
async function run(context) {
  ...
  const agentsIdeDir = path.join(context.projectRoot, '.antigravity', 'agents');
  ...
  if (!fs.existsSync(agentsIdeDir)) {
    return {
      check: name,
      status: 'WARN',
      message: 'IDE agents directory not found (.antigravity/agents/)',
      fixCommand: 'npx aiox-core install --force',
    };
  }
