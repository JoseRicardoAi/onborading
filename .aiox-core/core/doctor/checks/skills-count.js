 ...
 *
 * Counts skill directories in .antigravity/skills/ that contain SKILL.md.
 * PASS: >=6, WARN: 1-5, FAIL: 0 or directory missing.
 *
...
async function run(context) {
  const skillsDir = path.join(context.projectRoot, '.antigravity', 'skills');

  if (!fs.existsSync(skillsDir)) {
    return {
      check: name,
      status: 'FAIL',
      message: 'Skills directory not found (.antigravity/skills/)',
