 ...
 *
 * Validates .gemini/settings.json exists, deny rules count >= 40,
...
async function run(context) {
  const settingsPath = path.join(context.projectRoot, '.gemini', 'settings.json');

  if (!fs.existsSync(settingsPath)) {
    return {
      check: name,
      status: 'FAIL',
      message: 'settings.json not found (.gemini/settings.json)',
