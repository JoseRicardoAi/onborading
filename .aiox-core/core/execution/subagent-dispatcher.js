 ...
 * Supports multiple AI providers (Antigravity, Gemini) with routing and fallback.
...
class SubagentDispatcher extends EventEmitter {
  constructor(config = {}) {
    ...
    this.providerMapping = config.providerMapping || {
      // Tasks that benefit from Antigravity's deep reasoning
    ...
    // Default provider (from config or antigravity)
    this.defaultProvider = config.defaultProvider || 'antigravity';
   ...
   * @returns {string} - Provider name ('antigravity', 'gemini', or 'auto')
   */
  resolveProvider(task, agentId) {
    // Check for explicit @gemini or @antigravity tag in task
    if (task.provider) {
    ...
    if (task.tags && Array.isArray(task.tags)) {
      ...
      if (task.tags.includes('@antigravity') || task.tags.includes('antigravity')) {
        return 'antigravity';
      }
    }
    ...
    if (description.includes('@antigravity')) return 'antigravity';
  ...
  async spawnSubagent(agentId, task, context) {
    ...
    // Fallback to direct Antigravity CLI
    return this.executeAntigravity(prompt);
  }
  ...
  async executeWithProvider(prompt, providerName, task) {
    ...
    if (!provider) {
      ...
      // Fallback to legacy Antigravity execution
      return this.executeAntigravity(prompt);
    }
    ...
    if (!isAvailable) {
      ...
      const fallbackName = providerName === 'antigravity' ? 'gemini' : 'antigravity';
      ...
      // Last resort: legacy Antigravity
      return this.executeAntigravity(prompt);
    }
   ...
   * Runs both Antigravity and Gemini, returns based on parallelMode
   * @param {string} prompt - Prompt to execute
   * @param {Object} task - Original task
  ...
  async executeParallel(prompt, task) {
    ...
    const antigravity = this.getAIProvider('antigravity');
    const gemini = this.getAIProvider('gemini');

    // Check availability
    const [antigravityAvail, geminiAvail] = await Promise.all([
      antigravity?.checkAvailability() || false,
      gemini?.checkAvailability() || false,
    ]);

    if (!antigravityAvail && !geminiAvail) {
      throw new Error('No AI providers available');
    }

    if (!antigravityAvail) return this.executeWithSingleProvider(gemini, prompt, task);
    if (!geminiAvail) return this.executeWithSingleProvider(antigravity, prompt, task);
    ...
    const results = await Promise.allSettled([
      this.executeWithSingleProvider(antigravity, prompt, task),
      this.executeWithSingleProvider(gemini, prompt, task),
    ]);

    const antigravityResult = results[0].status === 'fulfilled' ? results[0].value : null;
    const geminiResult = results[1].status === 'fulfilled' ? results[1].value : null;

    this.emit('parallel_execution_complete', {
      taskId: task.id,
      duration: Date.now() - startTime,
      antigravitySuccess: !!antigravityResult?.success,
      geminiSuccess: !!geminiResult?.success,
    });

    // Select result based on mode
    return this.selectParallelResult(antigravityResult, geminiResult, task);
  }

  /**
   * Select result from parallel execution based on mode
   * @param {Object|null} antigravityResult - Antigravity result
   * @param {Object|null} geminiResult - Gemini result
   * @param {Object} task - Original task
   * @returns {Object} - Selected result
   */
  selectParallelResult(antigravityResult, geminiResult, task) {
    switch (this.parallelMode) {
      case 'race':
        // Return first successful result
        return antigravityResult?.success ? antigravityResult : geminiResult || antigravityResult;

      case 'consensus':
        // Both must succeed and be similar
        if (antigravityResult?.success && geminiResult?.success) {
        ...
        return antigravityResult?.success ? antigravityResult : geminiResult;

      case 'best-of':
        // Return longer/more complete response (simple heuristic)
        if (antigravityResult?.success && geminiResult?.success) {
          const antigravityLen = antigravityResult.output?.length || 0;
          const geminiLen = geminiResult.output?.length || 0;
          return antigravityLen >= geminiLen ? antigravityResult : geminiResult;
        }
        return antigravityResult?.success ? antigravityResult : geminiResult;

      case 'fallback':
      default:
        // Antigravity primary, Gemini fallback
        return antigravityResult?.success ? antigravityResult : geminiResult || antigravityResult;
    }
  }
  ...
  /**
   * Execute prompt via Antigravity CLI
   * @param {string} prompt - Prompt to execute
   * @returns {Promise<Object>} - Execution result
   */
  executeAntigravity(prompt) {
    return new Promise((resolve, reject) => {
      const args = ['--print', '--dangerously-skip-permissions'];
      const escapedPrompt = prompt.replace(/'/g, "'\\''");
      const fullCommand = `echo '${escapedPrompt}' | antigravity ${args.join(' ')}`;
  ...
  /**
   * Extract modified files from Antigravity output
   * @param {string} output - Antigravity output
  ...
  getProviderStats() {
    ...
    const stats = {
      antigravity: { total: 0, success: 0, failed: 0, avgDuration: 0 },
    ...
    for (const dispatch of dispatches) {
      ...
      const provider = dispatch.provider || 'antigravity';
