 ...
 * 4. AIResolver - Uses Antigravity for complex conflict resolution
 * 5. MergeOrchestrator - Coordinates the complete pipeline
 *
 * Based on Auto-Antigravity's merge system architecture.
 */
...
class AIResolver {
  ...
  /**
   * Resolve a conflict using AI (Antigravity CLI)
   * @param {Object} conflict - Conflict region
  ...
  async resolveConflict(conflict, baseContent, taskSnapshots) {
    ...
    try {
      const response = await this.callAntigravity(prompt);
      this.callCount++;
  ...
  /**
   * Build the merge prompt for Antigravity
   */
...
\`\`\``;
  ...
  /**
   * Call Antigravity CLI for merge resolution
   */
  async callAntigravity(prompt) {
    return new Promise((resolve, reject) => {
      try {
        // Use Antigravity CLI in print mode
        const result = execSync(
          `antigravity --print --dangerously-skip-permissions -p "${prompt.replace(/"/g, '\\"').replace(/\n/g, '\\n')}"`,
          {
            encoding: 'utf8',
