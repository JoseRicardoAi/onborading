...
const DEFAULT_CONFIG = {
  ...
  // Antigravity CLI
  antigravityModel: null, // Use default model from CLI config
  antigravityMaxTokens: 16000,
...
class BuildOrchestrator extends EventEmitter {
  ...
  async phasePlan(ctx) {
    ...
    // Generate plan using Antigravity
    if (ctx.config.dryRun) {
  ...
  async phaseExecute(ctx) {
    ...
    // Create executor that uses Antigravity CLI
    const executor = async (subtask, execCtx) => {
      return this.executeSubtaskWithAntigravity(subtask, execCtx, ctx);
    };
    ...
    const loop = new AutonomousBuildLoop({
      ...
      executor, // THE KEY: inject our Antigravity executor
    });
  ...
  /**
   * Execute a single subtask using Antigravity CLI
   * THIS IS WHERE THE MAGIC HAPPENS
   */
  async executeSubtaskWithAntigravity(subtask, execCtx, buildCtx) {
    const workDir = buildCtx.worktree?.path || this.rootPath;

    // Build the prompt for Antigravity
    ...
    try {
      // Execute using Antigravity CLI
      const result = await this.runAntigravityCLI(prompt, workDir, buildCtx.config);
...
`;
  ...
  /**
   * Run Antigravity CLI with prompt
   */
  async runAntigravityCLI(prompt, workDir, config) {
    return new Promise((resolve, reject) => {
      ...
      if (config.antigravityModel) {
        args.push('--model', config.antigravityModel);
      }
      ...
      this.log(`Running Antigravity CLI in ${workDir}`, 'debug');
...
`;
  ...
  /**
   * Generate implementation plan using Antigravity
   */
  async generatePlan(ctx) {
    // For now, return a basic plan structure
    // In a full implementation, this would use Antigravity to analyze the story
