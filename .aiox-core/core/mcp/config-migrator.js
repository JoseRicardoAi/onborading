...
function detectProjectConfig(projectRoot = process.cwd()) {
  const possiblePaths = [
    ...
    path.join(projectRoot, '.antigravity', 'mcp.json'),
  ...
  // Also check for legacy mcpServers format (from .antigravity.json style)
  const antigravityConfigPath = path.join(projectRoot, '.antigravity.json');
  if (fs.existsSync(antigravityConfigPath)) {
    try {
      const content = fs.readFileSync(antigravityConfigPath, 'utf8');
      const config = JSON.parse(content);
