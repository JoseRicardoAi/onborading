...
function collectHookStatus(projectRoot) {
  ...
  const settingsPath = path.join(projectRoot, '.antigravity', 'settings.local.json');
  ...
  try {
    if (fs.existsSync(settingsPath)) {
      ...
      hasHookRegistered = promptHooks.some((entry) => {
        ...
        // Nested format (Antigravity Code actual): { hooks: [{ type, command }] }
  ...
  const hookPath = path.join(projectRoot, '.antigravity', 'hooks', 'synapse-engine.cjs');
  ...
  if (hookExists) {
    try {
      ...
      checks.push({
        ...
        status: 'PASS',
        detail: `.antigravity/hooks/synapse-engine.cjs (${lineCount} lines, ${stat.size} bytes)`,
      });
    } catch (error) {
      checks.push({
        name: 'Hook file exists',
    ...
    checks.push({
      name: 'Hook file exists',
      status: 'FAIL',
      detail: '.antigravity/hooks/synapse-engine.cjs not found',
    });
  }
