...
class McpIntegrationCheck extends BaseCheck {
  ...
  async execute(context) {
    ...
    try {
      ...
      if (config.mcpServers) {
        mcpConfigs.found.push('.antigravity.json (global)');
        const servers = Object.keys(config.mcpServers);
    ...
    try {
      const localConfig = path.join(projectRoot, '.antigravity', 'settings.json');
      const content = await fs.readFile(localConfig, 'utf8');
      const config = JSON.parse(content);

      if (config.mcpServers) {
        mcpConfigs.found.push('.antigravity/settings.json');
        const servers = Object.keys(config.mcpServers);
