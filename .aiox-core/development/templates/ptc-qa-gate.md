...
ptc_type: bash-batch  # Fallback — true PTC not available in Antigravity Code CLI (ADR-7)
...
True PTC (API-level) would yield ~37% but is not available in Antigravity Code CLI.
