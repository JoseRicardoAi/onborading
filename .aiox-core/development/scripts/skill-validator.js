 ...
 * Antigravity Code and Gemini CLI environments.
 */
...
class SkillValidator {
  ...
  async validateAgentSkill(filePath) {
    ...
    try {
      ...
      this.validateAntigravityCompatibility(agentDef, result);
  ...
  /**
   * Validate Antigravity Code compatibility
   * @param {Object} agentDef - Agent definition
   * @param {Object} result - Result object to update
   */
  validateAntigravityCompatibility(agentDef, result) {
    // Antigravity Code uses .antigravity/commands/ structure
    // Check for any Antigravity-specific features that might not work in Gemini

    // Antigravity supports slash commands via Skill tool
    if (agentDef.commands) {
  ...
  generateReport(results) {
    ...
    if (results.agents.length > 0) {
      ...
      for (const agent of results.agents) {
        ...
        report += `- Antigravity Compatible: ${agent.antigravityCompatible ? 'Yes' : 'No'}\n`;
