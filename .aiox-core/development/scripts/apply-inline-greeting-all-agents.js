...
const ANTIGRAVITY_AGENTS_DIR = path.join(__dirname, '..', '..', '.antigravity', 'commands', 'AIOX', 'agents');
...
function updateAgent(agentFile) {
  ...
  try {
    ...
    // Sync to Antigravity commands directory
    const antigravityPath = path.join(ANTIGRAVITY_AGENTS_DIR, agentFile);
    fs.writeFileSync(antigravityPath, content);
...
function main() {
  ...
  if (results.updated > 0) {
    ...
    console.log('🔄 Files synchronized to .antigravity/commands/AIOX/agents/');
  }
}
