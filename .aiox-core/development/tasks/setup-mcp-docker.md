...
- campo: antigravity_integration
  tipo: boolean
  destino: Antigravity CLI configuration
  persistido: true
...
post-conditions:
  ...
  - [ ] Antigravity CLI connected
    tipo: post-condition
    blocker: true
    validação: Antigravity CLI shows docker-gateway in MCP list
    error_message: "Antigravity CLI not connected to MCP Gateway"
```
...
acceptance-criteria:
  ...
  - [ ] Antigravity CLI can call MCP tools
...
- **Tool:** Antigravity CLI CLI
  - **Purpose:** Integration verification
  - **Source:** antigravity command
...
### 5. Configure Antigravity CLI (HTTP Transport)
...
- stdio: Antigravity CLI spawns gateway → 30s timeout before init completes
...
### 7. Test in Antigravity CLI

After restarting Antigravity CLI:
```
/mcp
...
```bash
cp ~/.antigravity.json ~/.antigravity.json.backup-pre-docker-mcp
```
...
### Step 3: Remove 1MCP from Antigravity Config
```json
// ~/.antigravity.json - REMOVE these entries
{
  "mcpServers": {
    // "1mcp-dev": { ... },     // REMOVE
...
```json
// ~/.antigravity.json - ADD this entry (HTTP, NOT stdio!)
{
  "mcpServers": {
    "docker-gateway": {
      "type": "http",
...
- [ ] Antigravity CLI configured with HTTP transport
- [ ] `/mcp` in Antigravity CLI shows docker-gateway connected
...
🔗 Antigravity CLI: Connected via HTTP to docker-gateway
📊 Token Usage: ~10-15k tokens (minimal) / ~20-25k (full)

📁 Configuration:
   • Gateway service: .docker/mcp/gateway-service.yml
   • Antigravity config: ~/.antigravity.json (HTTP transport)

Next steps:
1. Restart Antigravity CLI to connect
...
changelog:
  ...
  2.0.0:
    ...
    - Added: Health check before Antigravity CLI connection
  1.0.0:
