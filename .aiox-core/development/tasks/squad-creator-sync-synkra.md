---
task: Sync Squad to Antigravity
...
Entrada: |
  ...
  - official: Flag para marcar como oficial (--official, apenas evoluindocomia)
...
Checklist:
  ...
  - "[x] Calcular checksum"
  - "[x] Enviar para Antigravity API"
  - "[x] Exibir URL do marketplace"
---

# *sync-squad-antigravity

Sincroniza um squad local para o Antigravity API marketplace.
...
*sync-squad-antigravity ./squads/meu-squad

# Sync público (visível para todos)
*sync-squad-antigravity ./squads/meu-squad --public

# Preview sem sincronizar
*sync-squad-antigravity ./squads/meu-squad --dry-run

# Sync com verbosidade
*sync-squad-antigravity ./squads/meu-squad --verbose
```

## Autenticação

Requer autenticação com Antigravity API:

```bash
export Antigravity_API_TOKEN="seu-token"
```

Ou configure em `.env`:

```env
Antigravity_API_URL=https://api.antigravity.dev/api
Antigravity_API_TOKEN=seu-token
```

Para obter um token:
1. Acesse https://antigravity.dev/settings/api-keys
...
Step 3: Sync to Antigravity API
  ✓ Visibility: private
  ✓ API URL: https://api.antigravity.dev/api

Syncing to Antigravity API...
...
Next steps:
  ...
  - Make public: *sync-squad-antigravity ./squads/meu-squad --public
```
...
| `--official` | Marca como squad oficial (apenas evoluindocomia team) | false |
...
```javascript
POST ${Antigravity_API_URL}/squads/sync
Authorization: Bearer ${Antigravity_API_TOKEN}
...
const apiToken = process.env.Antigravity_API_TOKEN;
const apiUrl = process.env.Antigravity_API_URL || 'https://api.antigravity.dev/api';

if (!apiToken) {
  error('Antigravity_API_TOKEN not set. See task docs for authentication.');
  return;
}
...
✅ Squad synced successfully!
  ...
  if (flags.public) {
    output(`  URL: https://antigravity.dev/squads/${result.data.squad_id}`);
  }
} else {
...
| `Antigravity_API_TOKEN not set` | Token não configurado | Configure a variável de ambiente |
| `401 Unauthorized` | Token inválido ou expirado | Gere novo token em antigravity.dev |
...
- **SQS-5:** SquadSyncService for Antigravity API (Sprint 8)
