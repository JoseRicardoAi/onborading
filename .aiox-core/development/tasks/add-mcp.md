...
Add the new MCP to `.antigravity/rules/mcp-usage.md`:
...
⚠️ **The user MUST restart their Antigravity CLI session** for new MCP tools to be available.

```text
IMPORTANT: New MCP tools will NOT be available until you:
1. Close this Antigravity CLI session
2. Open a new Antigravity CLI session: `antigravity`
...
After user restarts Antigravity CLI, verify tools are accessible:

```bash
# In new Antigravity CLI session, ask an agent to use the new MCP
...
post-conditions:
  ...
  - [ ] AIOX documentation updated
    tipo: post-condition
    blocker: true
    validacao: .antigravity/rules/mcp-usage.md includes new MCP
    error_message: "Update mcp-usage.md with new MCP documentation"

  - [ ] User notified about session restart
    tipo: post-condition
    blocker: true
    validacao: User informed to restart Antigravity CLI session
    error_message: "Notify user: tools only available after session restart"
```

**CRITICAL NOTE:** Tools added to Docker MCP Toolkit are NOT immediately available to AIOX agents. The docker-gateway caches tools at Antigravity CLI startup. User MUST restart their Antigravity CLI session for new tools to appear.
