...
Multi-provider AI integration for AIOX. Supports Antigravity Code and Gemini CLI with automatic fallback and task-based routing.
...
├── antigravity-provider.js       # Antigravity Code implementation
...
const provider = getProviderForTask('security');        // Returns Antigravity (deeper reasoning)
```

### Direct Provider Access

```javascript
const { AntigravityProvider, GeminiProvider } = require('./ai-providers');

// Antigravity
const antigravity = new AntigravityProvider({ model: 'antigravity-3-5-sonnet' });
const response = await antigravity.execute('Explain this function');
...
console.log(status);
// {
//   antigravity: { available: true, version: '1.0.0', ... },
