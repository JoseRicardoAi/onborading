/**
 * @fileoverview Antigravity Code Provider
 *
 * AI Provider implementation for Anthropic's Antigravity Code CLI.
 * Wraps the `antigravity` CLI command for prompt execution.
 *
...
/**
 * Antigravity Code provider implementation
 *
...
class AntigravityProvider extends AIProvider {
  /**
   * Create a Antigravity provider
   * @param {Object} [config={}] - Provider configuration
   * @param {string} [config.model='antigravity-3-5-sonnet'] - Model to use
  ...
  constructor(config = {}) {
    super({
      ...
      options: {
        model: config.model || 'antigravity-3-5-sonnet',
  ...
  /**
   * Execute a prompt using Antigravity CLI
  ...
  async execute(prompt, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Spawn antigravity directly without shell interpolation (safer)
      ...
      const timeoutId = setTimeout(() => {
        child.kill('SIGTERM');
        reject(new Error(`Antigravity execution timed out after ${timeout}ms`));
      }, timeout);
      ...
      child.on('error', (error) => {
        clearTimeout(timeoutId);
        reject(new Error(`Antigravity spawn error: ${error.message}`));
      });
    });
  }
