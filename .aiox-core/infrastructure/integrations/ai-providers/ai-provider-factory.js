 ...
 *
 * Central factory for creating and managing AI providers (Antigravity, Gemini).
...
function loadConfig(projectRoot = process.cwd()) {
  ...
  try {
    ...
    cachedConfig = {
      ...
      antigravity: { ...DEFAULT_CONFIG.antigravity, ...userConfig?.antigravity },
 ...
 * @param {string} providerName - Provider name ('antigravity' or 'gemini')
...
function getProvider(providerName, config = null) {
  ...
  switch (providerName.toLowerCase()) {
    case 'antigravity':
      provider = new AntigravityProvider(providerConfig);
      break;
...
function getPrimaryProvider() {
  ...
  const primaryName = config.ai_providers?.primary || 'antigravity';
...
function getProviderForTask(taskType) {
  ...
  const providerName = routing[taskType] || config.ai_providers?.primary || 'antigravity';
...
async function getAvailableProviders() {
  const providers = [getProvider('antigravity'), getProvider('gemini')];
