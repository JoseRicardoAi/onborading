      ...
      const status = {
        ...
        worktrees: {
          'STORY-42': {
            ...
            branch: 'auto-antigravity/STORY-42',
          ...
          'STORY-43': {
            ...
            branch: 'auto-antigravity/STORY-43',
  ...
  describe('getWorktreesStatus', () => {
    ...
    it('should return worktree info with required fields', async () => {
      ...
      try {
        ...
        await execa('git', ['worktree', 'add', worktreePath, '-b', 'auto-antigravity/STORY-TEST'], {
          cwd: testRoot,
        });
        ...
        expect(worktrees['STORY-TEST'].branch).toBe('auto-antigravity/STORY-TEST');
        ...
        await execa('git', ['branch', '-D', 'auto-antigravity/STORY-TEST'], { cwd: testRoot });
      } catch (error) {
  ...
  describe('generateStatus with worktrees', () => {
    it('should include worktrees in generated status', async () => {
      ...
      try {
        ...
        await execa('git', ['worktree', 'add', worktreePath, '-b', 'auto-antigravity/STORY-GEN'], {
          cwd: testRoot,
        });
        ...
        // Cleanup
        await execa('git', ['worktree', 'remove', worktreePath, '--force'], { cwd: testRoot });
        await execa('git', ['branch', '-D', 'auto-antigravity/STORY-GEN'], { cwd: testRoot });
      } catch (error) {
        console.warn('Git not available, skipping test:', error.message);
        expect(true).toBe(true);
      }
