...
function runSyncValidate(ide, projectRoot) {
  ...
  const result = spawnSync(process.execPath, [script, 'validate', '--ide', ide, '--strict'], {
    cwd: projectRoot,
    encoding: 'utf8',
  });
  return {
    ...
    errors: result.status === 0
      ? []
      : [`Sync validation failed for ${ide}${result.error ? `: ${result.error.message}` : ''}`],
    warnings: [],
...
function getEnabledIdeTargets(projectRoot) {
  ...
  try {
    ...
    const targets = config && config.ideSync && config.ideSync.targets;
    ...
    return new Set(
      Object.entries(targets)
        .filter(([, targetConfig]) => targetConfig && targetConfig.enabled)
        .map(([ide]) => ide),
    );
  } catch (error) {
    return null;
...
function runParityValidation(options = {}, deps = {}) {
  ...
  const enabledTargets = getEnabledIdeTargets(projectRoot);
  const allChecks = [
    { ide: 'antigravity-code', id: 'antigravity-sync', exec: () => runSync('antigravity-code', projectRoot) },
    { ide: 'antigravity-code', id: 'antigravity-integration', exec: () => runAntigravityIntegration({ projectRoot }) },
    { ide: 'codex', id: 'codex-sync', exec: () => runSync('codex', projectRoot) },
    { ide: 'codex', id: 'codex-integration', exec: () => runCodexIntegration({ projectRoot }) },
    { ide: 'gemini', id: 'gemini-sync', exec: () => runSync('gemini', projectRoot) },
    { ide: 'gemini', id: 'gemini-integration', exec: () => runGeminiIntegration({ projectRoot }) },
    { ide: 'cursor', id: 'cursor-sync', exec: () => runSync('cursor', projectRoot) },
    { ide: 'github-copilot', id: 'github-copilot-sync', exec: () => runSync('github-copilot', projectRoot) },
    { ide: 'antigravity', id: 'antigravity-sync', exec: () => runSync('antigravity', projectRoot) },
    { id: 'codex-skills', exec: () => runCodexSkills({ projectRoot, strict: true, quiet: true }) },
    { id: 'paths', exec: () => runPaths({ projectRoot }) },
  ];
  const checks = enabledTargets
    ? allChecks.filter((check) => !check.ide || enabledTargets.has(check.ide))
    : allChecks;
  ...
  const scopedContract = contract && enabledTargets
    ? {
        ...contract,
        ide_matrix: (contract.ide_matrix || []).filter((ide) => enabledTargets.has(ide.ide)),
      }
    : contract;
  const contractViolations = validateCompatibilityContract(scopedContract, resultById, {
    docsPath,
    docsPathRelative,
  });
  const contractSummary = contract
    ? {
        release: scopedContract.release || null,
