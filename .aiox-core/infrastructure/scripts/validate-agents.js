...
function validateAgentFormat(agents) {
  ...
  for (const agent of agents) {
    ...
    // Check autoAntigravity section
    if (!parsed.autoAntigravity) {
      warnings.push({
        type: 'MISSING_AUTOANTIGRAVITY',
        agent: id,
        message: `Missing autoAntigravity section in ${file} (V2 format)`,
        suggestion: `Add autoAntigravity section with version: '3.0'`,
      });
    }
