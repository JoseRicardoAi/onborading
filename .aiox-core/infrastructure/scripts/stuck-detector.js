 ...
 *
 * Detects when Auto-Antigravity execution is stuck in circular patterns
 ...
 * - AC7: Configurable via autoAntigravity.recovery.maxAttempts (default: 3)
 *
...
const DEFAULT_CONFIG = {
  maxAttempts: 3, // AC7: Default from autoAntigravity.recovery.maxAttempts
...
async function loadConfig(configPath) {
  ...
  try {
    ...
    // Extract stuck detector config from autoAntigravity section
    const autoAntigravity = config.autoAntigravity || {};
    const recovery = autoAntigravity.recovery || {};
...
async function main() {
  ...
  if (args.length === 0 || args.includes('--help') || args.includes('-h')) {
    console.log(`
${chalk.bold('Stuck Detector')} - AIOX Auto-Antigravity Recovery System (Story 5.2)
...
${chalk.cyan('Acceptance Criteria:')}
  ...
  AC5: Escalates to human with complete context
  AC6: Suggests alternative approaches based on errors
  AC7: Configurable via autoAntigravity.recovery.maxAttempts
