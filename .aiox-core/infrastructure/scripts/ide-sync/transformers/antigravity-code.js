/**
 * Antigravity Code Transformer - Full markdown with YAML (identity transform)
 * @story 6.19 - IDE Command Auto-Sync System
 *
 * Format: Full markdown file with embedded YAML block
 * Target: .antigravity/commands/AIOX/agents/*.md
 */

/**
 * Transform agent data to Antigravity Code format
 * For Antigravity Code, we use the full original file (identity transform)
 * @param {object} agentData - Parsed agent data from agent-parser
 * @returns {string} - Transformed content
 */
function transform(agentData) {
  // Antigravity Code uses the full original file
  if (agentData.raw) {
