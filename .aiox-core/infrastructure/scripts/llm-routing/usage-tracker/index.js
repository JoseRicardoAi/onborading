 ...
 *
 * A lightweight HTTP proxy that tracks API usage per alias (e.g., "antigravity-free").
 ...
 * @param {string} alias - API key alias (e.g., "antigravity-free")
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    case 'serve': {
      ...
      const alias = args.find(a => a.startsWith('--alias='))?.split('=')[1] || 'antigravity-free';
...
Commands:
  start [--port=8787] [--alias=antigravity-free]  Start proxy server
  usage [alias]                               Show usage statistics
  reset                                       Reset usage data (with backup)
  export [filename]                           Export usage data to JSON

Examples:
  node index.js start --alias=antigravity-free
  node index.js usage antigravity-free
