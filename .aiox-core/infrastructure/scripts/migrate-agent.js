 ...
 * Story 2.4: Migrates agents to V3 format with Auto-Antigravity capabilities
 *
 * Usage:
...
const AGENT_CAPABILITIES = {
  ...
  sm: {
    // Scrum Master doesn't have specific Auto-Antigravity capabilities
  },
  'aiox-master': {
    specPipeline: {
      canGather: true,
...
/**
 * Check if agent already has V3 autoAntigravity section
 */
function isAlreadyV3(parsed) {
  return parsed?.autoAntigravity?.version === '3.0';
}

/**
 * Generate autoAntigravity section for an agent
 */
function generateAutoAntigravitySection(agentId) {
  const capabilities = AGENT_CAPABILITIES[agentId] || {};

  const autoAntigravity = {
    version: '3.0',
  ...
  if (capabilities.execution) {
    ...
    if (capabilities.execution.selfCritique) {
      autoAntigravity.execution.selfCritique = capabilities.execution.selfCritique;
    }
  }

  if (capabilities.recovery) {
    autoAntigravity.recovery = capabilities.recovery;
  }

  if (capabilities.qa) {
    autoAntigravity.qa = capabilities.qa;
  }

  if (capabilities.memory) {
    autoAntigravity.memory = {
  ...
  if (capabilities.worktree) {
    autoAntigravity.worktree = capabilities.worktree;
  }

  return autoAntigravity;
}

/**
 * Convert autoAntigravity object to YAML string with proper indentation
 */
function autoAntigravityToYaml(autoAntigravity) {
  return yaml.dump({ autoAntigravity }, { indent: 2, lineWidth: -1 }).trim();
}

/**
 * Insert autoAntigravity section into markdown content
 */
function insertAutoAntigravitySection(content, autoAntigravityYaml) {
  ...
  // Insert autoAntigravity section before the closing ```
 ...
 */
function generateDiff(agentId, autoAntigravity) {
  const lines = [];
  ...
  lines.push(`═══════════════════════════════════════════════════════════`);
  lines.push('');
  lines.push('+ Added autoAntigravity section:');
  lines.push('');

  const yamlStr = autoAntigravityToYaml(autoAntigravity);
  yamlStr.split('\n').forEach((line) => {
    lines.push(`  + ${line}`);
  });
...
async function migrateAgent(rootPath, agentId, options = {}) {
  ...
  // Generate autoAntigravity section
  const autoAntigravity = generateAutoAntigravitySection(agentId);
  const autoAntigravityYaml = autoAntigravityToYaml(autoAntigravity);
  ...
  if (dryRun) {
    return {
      success: true,
      dryRun: true,
      diff: generateDiff(agentId, autoAntigravity),
      autoAntigravity,
    };
  }
  ...
  // Insert autoAntigravity section
  const { content: newContent, error: insertError } = insertAutoAntigravitySection(
    content,
    autoAntigravityYaml
  );
...
module.exports = {
  migrateAgent,
  listAgents,
  generateAutoAntigravitySection,
  AGENT_CAPABILITIES,
};
