...
// In project-dev: pro is installed via npm as @aiox-antigravity/pro
function resolveLicensePath() {
  ...
  // 2. Try node_modules/@aiox-antigravity/pro/license (project-dev mode)
  try {
    const proPkg = require.resolve('@aiox-antigravity/pro/package.json');
    ...
    // @aiox-antigravity/pro not installed via npm
  }

  // 3. Try project root node_modules (fallback)
  const projectRoot = process.cwd();
  const cwdPath = path.join(projectRoot, 'node_modules', '@aiox-antigravity', 'pro', 'license');
...
function loadLicenseModules() {
  try {
    ...
    console.error('Install AIOX Pro: npm install @aiox-antigravity/pro');
    process.exit(1);
  }
}
...
async function activateAction(options) {
  ...
  try {
    ...
    const proSourceDir = path.join(projectRoot, 'node_modules', '@aiox-antigravity', 'pro');
    ...
    if (fs.existsSync(proSourceDir)) {
      ...
      console.log('Note: @aiox-antigravity/pro package not found in node_modules.');
...
function statusAction() {
  ...
  if (!cache) {
    ...
    console.log('  Purchase: https://antigravity.ai/pro');
    console.log('');
    return;
  }
...
/**
 * Setup and verify @aiox-antigravity/pro installation.
 *
 * Since @aiox-antigravity/pro is published on the public npm registry,
...
async function setupAction(options) {
  ...
  if (options.verify) {
    // Verify-only mode
    console.log('Verifying @aiox-antigravity/pro installation...\n');

    try {
      const { execSync } = require('child_process');
      const result = execSync('npm ls @aiox-antigravity/pro --json', {
        stdio: 'pipe',
        timeout: 15000,
      });
      const parsed = JSON.parse(result.toString());
      const deps = parsed.dependencies || {};
      if (deps['@aiox-antigravity/pro']) {
        console.log(`✅ @aiox-antigravity/pro@${deps['@aiox-antigravity/pro'].version} is installed`);
      } else {
        console.log('❌ @aiox-antigravity/pro is not installed');
        console.log('');
        console.log('Install with:');
        console.log('  npm install @aiox-antigravity/pro');
      }
    } catch {
      console.log('❌ @aiox-antigravity/pro is not installed');
      console.log('');
  ...
  // Install mode
  console.log('@aiox-antigravity/pro is available on the public npm registry.');
  console.log('No special tokens or configuration needed.\n');

  console.log('Installing @aiox-antigravity/pro...\n');

  try {
    const { execSync } = require('child_process');
    execSync('npm install @aiox-antigravity/pro', {
      stdio: 'inherit',
      timeout: 120000,
    });
    console.log('\n✅ @aiox-antigravity/pro installed successfully!');
  } catch (error) {
  ...
  console.log('');
  console.log('Documentation: https://antigravity.ai/pro/docs');
  console.log('');
}
...
function createProCommand() {
  ...
  proCmd
    .command('setup')
    .description('Install and verify @aiox-antigravity/pro')
