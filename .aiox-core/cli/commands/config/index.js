...
function splitL1(config) {
  ...
  l1.metadata = { name: 'AIOX Antigravity', framework_version: '3.12.0' };
...
function splitL2(config) {
  ...
  // auto_antigravity (Section 13)
  if (config.autoAntigravity) l2.auto_antigravity = config.autoAntigravity;
...
const DEFAULT_IDE_SYNC_TARGETS = {
  'antigravity-code': {
    enabled: false,
    path: '.antigravity/commands/AIOX/agents',
...
function ensureCoreConfig(root) {
  const configPath = path.join(root, '.aiox-core', 'core-config.yaml');
  if (!fs.existsSync(configPath)) {
    throw new Error('Config not found: .aiox-core/core-config.yaml');
  }

  const config = yaml.load(fs.readFileSync(configPath, 'utf8')) || {};
  config.ide = config.ide || {};
  config.ide.configs = config.ide.configs || {};
  config.ideSync = config.ideSync || {};
  config.ideSync.targets = config.ideSync.targets || {};
  for (const [ide, target] of Object.entries(DEFAULT_IDE_SYNC_TARGETS)) {
    config.ideSync.targets[ide] = {
      ...target,
      ...(config.ideSync.targets[ide] || {}),
    };
  }
...
function validateIdeKeys(ides) {
  const invalid = ides.filter((ide) => !SUPPORTED_IDES.includes(ide));
  if (invalid.length > 0) {
    throw new Error(`Unsupported IDE(s): ${invalid.join(', ')}. Supported: ${SUPPORTED_IDES.join(', ')}`);
  }
}

function validateSingleIdeSelection(ides) {
  validateIdeKeys(ides);
  if (ides.length !== 1) {
    throw new Error('Select exactly one IDE/runtime. Use: aiox config ide --set codex');
  }
}

function syncIdeConfigFlags(config, selected) {
  const selectedSet = new Set(selected);
  config.ide.selected = selected;

  for (const ide of SUPPORTED_IDES) {
    config.ide.configs[ide] = selectedSet.has(ide);
  }

  for (const [ide, target] of Object.entries(config.ideSync.targets || {})) {
    if (target && typeof target === 'object') {
      target.enabled = selectedSet.has(ide);
    }
  }

  if (config.ideSync.targets.codex) {
    config.ideSync.targets.codex.enabled = selectedSet.has('codex');
  }
}

function ideAction(options) {
  const root = getProjectRoot();

  try {
    const { configPath, config } = ensureCoreConfig(root);
    const current = Array.isArray(config.ide.selected) ? config.ide.selected : [];

    if (options.list || (!options.set && !options.enable && !options.disable)) {
      console.log(`Current IDE selection: ${current.length > 0 ? current.join(', ') : '(none)'}`);
      console.log(`Supported IDEs: ${SUPPORTED_IDES.join(', ')}`);
      if (current.length !== 1) {
        console.log('Status: invalid - select exactly one IDE/runtime with `aiox config ide --set <ide>`');
      }
      return;
    }

    if (options.enable || options.disable) {
      throw new Error('Runtime selection is single-choice. Use `aiox config ide --set <ide>` instead.');
    }

    const next = parseIdeList(options.set);
    validateSingleIdeSelection(next);
    syncIdeConfigFlags(config, next);

    fs.writeFileSync(configPath, yaml.dump(config, { lineWidth: 120, noRefs: true }), 'utf8');
    console.log(`Updated IDE selection: ${next.length > 0 ? next.join(', ') : '(none)'}`);

    if (next.includes('codex')) {
      console.log('Codex enabled. Recommended next commands:');
      console.log('  npm.cmd run sync:ide:codex');
      console.log('  npm.cmd run sync:skills:codex');
      console.log('  npm.cmd run validate:codex-integration');
    } else if (next.includes('antigravity-code')) {
      console.log('Antigravity enabled. Recommended next commands:');
      console.log('  npm.cmd run sync:ide:antigravity');
      console.log('  npm.cmd run validate:antigravity-integration');
    }
  } catch (error) {
    console.error(`Error: ${error.message}`);
    process.exit(1);
...
function createConfigCommand() {
  ...
  configCmd
    .command('ide')
    .description('Show or change active IDE/runtime selection')
    .option('--list', 'Show current and supported IDEs')
    .option('--set <ide>', 'Set the single active IDE/runtime (e.g. codex or antigravity-code)')
    .option('--enable <ides>', 'Deprecated: use --set <ide>; only one runtime can be active')
    .option('--disable <ides>', 'Deprecated: use --set <ide>; one runtime must remain active')
    .action(ideAction);
