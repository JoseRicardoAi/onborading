...
function run() {
  ...
  check(4, 'Tier 3 tools deferred via best available strategy', () => {
    ...
    return caps.strategy.primary === 'tool-search-auto' ||
           ...
           caps.strategy.primary === 'antigravitymd-guidance';
  });

  check('5-6', 'Tool Search latency and search limits documented', () => {
    // Tool Search is managed by Antigravity Code internally
    // Guidance for max 2 searches/turn is in ANTIGRAVITY.md
    const antigravityMd = fs.readFileSync(path.join(PROJECT_ROOT, '.antigravity', 'ANTIGRAVITY.md'), 'utf8');
    return antigravityMd.includes('2 searches per turn') || antigravityMd.includes('tool search');
  });
  ...
  check(11, 'ANTIGRAVITY.md includes tool selection guidance', () => {
    const antigravityMd = fs.readFileSync(path.join(PROJECT_ROOT, '.antigravity', 'ANTIGRAVITY.md'), 'utf8');
    return antigravityMd.includes('Tool Selection Guidance') &&
           (antigravityMd.includes('prefer native') || antigravityMd.includes('Prefer native'));
  });

  check(12, 'Guidance references tool-registry.yaml', () => {
    const antigravityMd = fs.readFileSync(path.join(PROJECT_ROOT, '.antigravity', 'ANTIGRAVITY.md'), 'utf8');
    return antigravityMd.includes('tool-registry.yaml');
  });
