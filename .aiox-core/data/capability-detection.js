...
// =============================================================================
// Detects Antigravity Code runtime capabilities for token optimization decisions.
...
function detectToolSearch() {
  // Check Antigravity Code cached features for tool search availability
  const antigravityJsonPath = path.join(os.homedir(), '.antigravity.json');
  try {
    const config = JSON.parse(fs.readFileSync(antigravityJsonPath, 'utf8'));
    ...
    return {
      ...
      detectionMethod: 'antigravity-json-feature-flag'
    };
  } catch {
    return {
      available: false,
...
function detectDeferLoading() {
  // defer_loading is API-only (Python SDK mcp_toolset), NOT available in Antigravity Code CLI
  return {
    available: false,
    reason: 'defer_loading is API-only (Python SDK). Not exposed in Antigravity Code CLI.',
...
function detectGlobalMcps() {
  ...
  // Direct MCPs in Antigravity Code (playwright, etc.)
  // These are registered via `antigravity mcp add` at user scope
  // Detection: check ~/.antigravity/settings.json or known conventions
  const settingsPath = path.join(os.homedir(), '.antigravity', 'settings.json');
  try {
    ...
    if (settings.mcpServers) {
      for (const [name, cfg] of Object.entries(settings.mcpServers)) {
        servers.push({
          ...
          source: '~/.antigravity/settings.json'
        });
      }
    }
  } catch {
...
function determineStrategy(toolSearch, deferLoading, dockerGateway) {
  ...
  // 3. Fallback 2: ANTIGRAVITY.md guidance — instruct to prefer native tools

  if (toolSearch.available) {
    return {
      primary: 'tool-search-auto',
      description: 'Antigravity Code Tool Search is active. Tier 3 MCP tools are automatically deferred via tool_search.',
      fallbacks: ['mcp-discipline', 'antigravitymd-guidance']
    };
  }
  ...
  if (dockerGateway.available) {
    return {
      ...
      fallbacks: ['antigravitymd-guidance']
    };
  }

  return {
    primary: 'antigravitymd-guidance',
    description: 'Neither Tool Search nor Docker Gateway available. Using ANTIGRAVITY.md guidance for tool selection priority.',
    fallbacks: []
  };
}
...
function run() {
  ...
  const capabilities = {
    ...
    methodology: {
      toolSearchLatency: 'Managed internally by Antigravity Code — not programmatically measurable. Guidance-level enforcement via ANTIGRAVITY.md.',
