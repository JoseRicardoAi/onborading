 ...
 *
 * Write hooks once, run on both Antigravity Code and Gemini CLI.
 */
 ...
 */
const EVENT_MAPPING = {
  // AIOX Event -> { antigravity: AntigravityEvent, gemini: GeminiEvent }
  sessionStart: { antigravity: null, gemini: 'SessionStart' },
  beforeAgent: { antigravity: 'PreToolUse', gemini: 'BeforeAgent' },
  beforeTool: { antigravity: 'PreToolUse', gemini: 'BeforeTool' },
  afterTool: { antigravity: 'PostToolUse', gemini: 'AfterTool' },
  sessionEnd: { antigravity: 'Stop', gemini: 'SessionEnd' },
};
...
class UnifiedHook {
  ...
  /**
   * Get Antigravity Code hook configuration
   * @returns {Object|null} Antigravity hook config or null if not supported
   */
  toAntigravityConfig() {
    const antigravityEvent = EVENT_MAPPING[this.event]?.antigravity;
    if (!antigravityEvent) return null;
    ...
    //   handler: `node .aiox-core/hooks/unified/runners/antigravity-runner.js ${this.name}`,
    // };
  }
 ...
 * @property {string} provider - Current provider ('antigravity' or 'gemini')
 */
...
function createContext(provider) {
  ...
  return {
    projectDir: isGemini
      ...
      : process.env.ANTIGRAVITY_PROJECT_DIR || process.cwd(),
    sessionId: isGemini
      ? process.env.GEMINI_SESSION_ID || `session-${Date.now()}`
      : process.env.ANTIGRAVITY_SESSION_ID || `session-${Date.now()}`,
    provider,
  };
}
