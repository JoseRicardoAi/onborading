...
#   ANTIGRAVITY_CMD        - Antigravity CLI command (default: antigravity)
#
# Exit Codes:
#   0 - Success
#   1 - Invalid arguments
...
ANTIGRAVITY_CMD="${ANTIGRAVITY_CMD:-antigravity}"
...
spawn_terminal() {
  ...
  local agent_cmd="${ANTIGRAVITY_CMD}"

  # Add agent activation
  agent_cmd="${agent_cmd} --print-only"  # Just for testing, real impl would use actual antigravity flags

  # For now, we'll create a simpler command that demonstrates the concept
  # The actual antigravity CLI integration will depend on how antigravity accepts agent/task args
  ...
  # full_cmd+="${ANTIGRAVITY_CMD} @${AGENT} *${TASK} ${PARAMS}"
