...
# =============================================================================
# Launches multiple Antigravity Code instances in tmux for parallel story development
...
declare -a STORIES=(
    "0.3|Layout Shell|docs/stories/aiox-dashboard/epic-0-foundation.md|apps/dashboard/src/components/layout/|Create Sidebar.tsx, AppShell.tsx following Auto-Antigravity layout"
...
if ! command -v antigravity &> /dev/null; then
    echo -e "${RED}❌ antigravity CLI not found${NC}"
    exit 1
fi
...
# =============================================================================
# PHASE 1: START ANTIGRAVITY IN ALL PANES
# =============================================================================

echo -e "${YELLOW}[1/3]${NC} Starting Antigravity Code in all panes..."
...
for ((i=0; i<NUM_WORKERS; i++)); do
    ...
    # Set environment and start Antigravity
    ...
    tmux send-keys -t "$SESSION_NAME:0.$i" "antigravity --dangerously-skip-permissions" Enter
    sleep 2  # Stagger launches
done

echo -e "   ${GREEN}✓${NC} Antigravity launched in $NUM_WORKERS panes"
echo -e "   ${YELLOW}⏳${NC} Waiting for Antigravity to initialize (10s)..."
sleep 10
