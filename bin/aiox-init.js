...
/**
 * AIOX-Antigravity Installation Wizard v5 (LEGACY)
 ...
 *
 * Migration path:
 * - Use `npx aiox-antigravity` which routes through bin/aiox.js to the new wizard
 * - Do NOT call this file directly
 *
 * Supported IDEs (4 total):
 * - Antigravity Code, Cursor, Gemini CLI, GitHub Copilot
 */
...
const SUBTITLE = chalk.magenta('AIOX Antigravity • Universal AI Agent Framework');
...
async function main() {
  ...
  if (!context) {
    ...
    if (!fs.existsSync(packageJsonPath)) {
      ...
      const defaultPackage = {
        ...
        version: '1.0.0',
        description: 'AIOX-Antigravity project',
        main: 'index.js',
  ...
  const { installMode } = await inquirer.prompt([
    {
      type: 'list',
      name: 'installMode',
      message: chalk.white('How are you using AIOX-Antigravity?'),
      choices: [
        {
  ...
  const { ides } = await inquirer.prompt([
    {
      ...
      choices: [
        {
          name: '  Antigravity Code ' + chalk.blue('(v4)') + chalk.gray(' - Recommended'),
          value: 'antigravity',
          checked: true,
        },
  ...
  if (ides.includes('antigravity')) {
    cliToolsToCheck.push({ ide: 'antigravity', command: 'antigravity', name: 'Antigravity Code', npm: '@anthropic-ai/antigravity-code' });
  }
      ...
      if (installClis) {
        for (const tool of missingTools) {
          ...
          try {
            ...
            if (tool.ide === 'antigravity') {
              console.log(chalk.gray('  Run `antigravity` to authenticate with your Anthropic account'));
  ...
  if (!ides.includes('none')) {
    ...
    const ideRulesMap = {
      antigravity: { source: 'antigravity-rules.md', target: '.antigravity/ANTIGRAVITY.md' },
    ...
    // Step 2: Install AIOX CORE agents and tasks for Antigravity Code
    ...
    if (ides.includes('antigravity')) {
      ...
      if (cachedAgentFiles.length > 0) {
        ...
        console.log(chalk.green('✓') + ` Antigravity Code CORE agents installed (${cachedAgentFiles.length} agents)`);
      }

      if (cachedTaskFiles.length > 0) {
        await fse.copy(coreTasksSource, coreTasksTarget);
        console.log(chalk.green('✓') + ` Antigravity Code CORE tasks installed (${cachedTaskFiles.length} tasks)`);
      }

      // Create AIOX README for Antigravity Code
...
This directory contains the core AIOX-Antigravity agents and tasks.
...
This directory contains the core AIOX-Antigravity agent rules for Cursor.
...
`
  ...
  if (availableSquads.length > 0) {
    ...
    if (selectedSquads.length > 0) {
      ...
      for (const squad of selectedSquads) {
        ...
        // INS-2 Performance: Cache squad file lists once per squad (used by Antigravity, Cursor, etc.)
        ...
        // Install squad agents/tasks for Antigravity Code
        if (ides.includes('antigravity')) {
          const squadAntigravityTarget = path.join(context.projectRoot, '.antigravity', 'commands', squad);

          // Copy agents (using cached list)
          if (squadAgentFiles.length > 0) {
            const squadAgentsTarget = path.join(squadAntigravityTarget, 'agents');
            await fse.copy(squadAgentsSource, squadAgentsTarget);
            console.log(chalk.green('  ✓') + ` Antigravity Code ${squad} agents (${squadAgentFiles.length} agents)`);
          }

          // Copy tasks (using cached list)
          if (squadTaskFiles.length > 0) {
            const squadTasksTarget = path.join(squadAntigravityTarget, 'tasks');
            await fse.copy(squadTasksSource, squadTasksTarget);
            console.log(chalk.green('  ✓') + ` Antigravity Code ${squad} tasks (${squadTaskFiles.length} tasks)`);
          }

          // Copy README (using cached check)
          if (hasSquadReadme) {
            await fse.copy(squadReadmeSource, path.join(squadAntigravityTarget, 'README.md'));
          }
        }
  ...
  console.log(chalk.gray('═'.repeat(80)));
  console.log('');
  console.log(chalk.green.bold('✓ AIOX-Antigravity installation complete! 🎉'));
  console.log('');
  ...
  if (ides.includes('antigravity')) {
    console.log('  ' + chalk.dim('.antigravity/'));
    console.log('    ' + chalk.dim('├─ ANTIGRAVITY.md') + '        - Main configuration');
  ...
  if (ides.includes('antigravity')) {
    console.log('  ' + chalk.yellow('Antigravity Code:'));
    console.log('    • Use slash commands: /dev, /architect, /qa, /pm, /github-devops');
    console.log('    • Browse: .antigravity/commands/AIOX/agents/ for all available agents');
  }
...
async function updateGitIgnore(mode, projectRoot) {
  ...
  if (mode === 'project-development') {
    const frameworkRules = [
      '',
      '# AIOX-Antigravity Framework Files (auto-managed - do not edit)',
      '.aiox-core/',
      'node_modules/@aiox/',
      'outputs/minds/',
      '.aiox-installation-config.yaml',
      '# End AIOX-Antigravity auto-managed section',
      '',
    ];

    const hasFrameworkSection = gitignore.includes('# AIOX-Antigravity Framework Files');
...
/**
 * Setup global statusline for Antigravity Code (legacy installer version)
...
async function setupGlobalStatuslineLegacy(sourceCoreDir) {
  ...
  const globalSettingsPath = path.join(homeDir, '.antigravity', 'settings.json');
  ...
  // Target paths
  const scriptTarget = path.join(homeDir, '.antigravity', 'statusline-script.js');
  const hookTarget = path.join(homeDir, '.antigravity', 'hooks', 'track-agent.sh');

  try {
    await fse.ensureDir(path.join(homeDir, '.antigravity', 'hooks'));
    await fse.ensureDir(path.join(homeDir, '.antigravity', 'session-cache'));
