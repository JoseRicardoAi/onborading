...
async function runWizard(options = {}) {
  ...
  if (!fs.existsSync(wizardPath)) {
    ...
    console.error('Please ensure AIOX-Antigravity is installed correctly.');
    process.exit(1);
  }
...
function showHelp() {
  console.log(`
AIOX-Antigravity v${packageJson.version}
AI-Orchestrated System for Full Stack Development

USAGE:
  npx aiox-antigravity@latest              # Run installation wizard
  npx aiox-antigravity@latest install      # Install in current project
  npx aiox-antigravity@latest init <name>  # Create new project
  npx aiox-antigravity@latest update       # Update to latest version
  npx aiox-antigravity@latest validate     # Validate installation integrity
  npx aiox-antigravity@latest info         # Show system info
  npx aiox-antigravity@latest doctor       # Run diagnostics
  npx aiox-antigravity@latest --version    # Show version
  npx aiox-antigravity@latest --version -d # Show detailed version info
  npx aiox-antigravity@latest --help       # Show this help

UPDATE:
  aiox-antigravity update                    # Update to latest version
  aiox-antigravity update --check            # Check for updates without applying
  aiox-antigravity update --dry-run          # Preview what would be updated
  aiox-antigravity update --force            # Force update even if up-to-date
  aiox-antigravity update --verbose          # Show detailed output

VALIDATION:
  aiox-antigravity validate                    # Validate installation integrity
  aiox-antigravity validate --repair           # Repair missing/corrupted files
  aiox-antigravity validate --repair --dry-run # Preview repairs
  aiox-antigravity validate --detailed         # Show detailed file list

CONFIGURATION:
  aiox-antigravity config show                       # Show resolved configuration
  aiox-antigravity config show --debug               # Show with source annotations
  aiox-antigravity config diff --levels L1,L2        # Compare config levels
  aiox-antigravity config migrate                    # Migrate monolithic to layered
  aiox-antigravity config validate                   # Validate config files
  aiox-antigravity config init-local                 # Create local-config.yaml

SERVICE DISCOVERY:
  aiox-antigravity workers search <query>            # Search for workers
  aiox-antigravity workers search "json" --category=data
  aiox-antigravity workers search "transform" --tags=etl,data
  aiox-antigravity workers search "api" --format=json

EXAMPLES:
  # Install in current directory
  npx aiox-antigravity@latest

  # Install with minimal mode (only expansion-creator)
  npx aiox-antigravity-minimal@latest

  # Create new project
  npx aiox-antigravity@latest init my-project

  # Search for workers
  aiox-antigravity workers search "json csv"

For more information, visit: https://github.com/evoluindocomia/aiox-antigravity
`);
}
...
async function showVersion() {
  ...
  console.log(`AIOX-Antigravity v${packageJson.version}`);
  console.log('Package: aiox-antigravity');
  ...
  if (fs.existsSync(localVersionPath)) {
    try {
      ...
      if (versionInfo.version !== packageJson.version) {
        ...
        console.log('  Run \'npx aiox-antigravity update\' to update.');
      } else {
        console.log('\n✅ Up to date');
      }
    } catch (error) {
    ...
    console.log('  Run \'npx aiox-antigravity install\' to install AIOX in this project.');
  }
}

// Helper: Show system info
function showInfo() {
  console.log('📊 AIOX-Antigravity System Information\n');
...
function showUninstallHelp() {
  console.log(`
Usage: npx aiox-antigravity uninstall [options]
...
Examples:
  ...
  npx aiox-antigravity uninstall

  # Force uninstall without prompts
  npx aiox-antigravity uninstall --force

  # See what would be removed
  npx aiox-antigravity uninstall --dry-run

  # Uninstall but keep project data
  npx aiox-antigravity uninstall --keep-data
`);
}
...
function showDoctorHelp() {
  console.log(`
Usage: npx aiox-antigravity doctor [options]
...
Examples:
  ...
  npx aiox-antigravity doctor --fix

  # Preview what would be fixed
  npx aiox-antigravity doctor --fix --dry-run
`);
}
...
async function runUninstall(options = {}) {
  ...
  if (!quiet) {
    ...
    console.log('\n   To reinstall: npx aiox-antigravity install');
  }
}
...
function showInstallHelp() {
  console.log(`
Usage: npx aiox-antigravity install [options]
...
Smart Merge (Brownfield):
  When installing in a project with existing config files (.env, ANTIGRAVITY.md),
  AIOX can merge new settings while preserving your customizations.

  - .env files: Adds new variables, preserves existing values
  - ANTIGRAVITY.md: Updates AIOX sections, keeps your custom rules
...
Examples:
  ...
  npx aiox-antigravity install --force

  # Brownfield: merge configs automatically
  npx aiox-antigravity install --merge
...
function showInitHelp() {
  console.log(`
Usage: npx aiox-antigravity init <project-name> [options]
...
Examples:
  ...
  npx aiox-antigravity init my-project --template minimal
  npx aiox-antigravity init my-project --force --skip-install
  npx aiox-antigravity init . --template enterprise
`);
}
...
async function main() {
  switch (command) {
    ...
    case 'synapse':
      // SYNAPSE explicit CLI - hook-free runtime for Codex and other CLIs
      try {
        const { run } = require('../.aiox-core/cli/index.js');
        await run(process.argv);
      } catch (error) {
        console.error(`âŒ Synapse command error: ${error.message}`);
        process.exit(1);
      }
      break;
    ...
    case 'install': {
      ...
      if (!installOptions.quiet) {
        console.log('AIOX-Antigravity Installation\n');
      }
