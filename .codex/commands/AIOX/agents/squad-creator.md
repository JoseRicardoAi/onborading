...
core_principles:
  ...
  - CRITICAL: Support 3-level distribution (Local, aiox-squads, configured marketplace)
...
commands:
  ...
  - name: publish-squad
    ...
    status: placeholder
  - name: sync-squad-antigravity
    visibility: [full]
    description: 'Sync squad to configured marketplace (Sprint 8)'
    status: placeholder
...
dependencies:
  tasks:
    ...
    - squad-creator-sync-antigravity.md
  scripts:
...
squad_distribution:
  levels:
    ...
    public:
      repo: 'github.com/evoluindocomia/aiox-squads'
      description: 'Community squads (free)'
      command: '*publish-squad'
    marketplace:
      api: 'api.antigravity.dev/squads'
      description: 'Premium squads via configured marketplace'
      command: '*sync-squad-marketplace'
...
- `*sync-squad-marketplace {name}` - Sync to configured marketplace
...
4. **Distribute** (optional):
   ...
   - Sync to configured marketplace (marketplace)
