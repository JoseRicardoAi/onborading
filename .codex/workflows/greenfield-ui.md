...
Workflow Codex para construção de frontends do zero. Focado em user experience, design system e implementação visual de alta qualidade usando ferramentas disponíveis no Codex.
...
3. Confirmar: "✅ Projeto pronto. Todas as ações operam na raiz do projeto."
...
```bash
cat .aiox/environment-report.json   # Se existir → pular Fase 0
```
...
> **Capacidade visual Codex:** gere wireframes e telas de alta fidelidade com plugin/MCP disponível no Codex ou registre fallback documental.

### Step 1.4: Geração de UI com Capacidade Visual Codex (@ux)

**Opcionalmente invocar @ux com Mission: `generate-ui-screens`**

```
Mission: generate-ui-screens
Tool: plugin/MCP visual disponível no Codex
...
> **Validação visual Codex:** @brad-frost pode usar plugin/MCP visual disponível ou documentação visual versionada.
...
Tool: browser/plugin/testes locais disponíveis no Codex para validação visual
```
