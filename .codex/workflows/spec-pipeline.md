...
4. **Integração visual Codex:** Se a seção `stitch-ui-guidelines` estiver habilitada no PRD, gerar obrigatoriamente o arquivo `docs/architecture/ui-guidelines.yaml` com as restrições e design system definidos. Use plugin/MCP visual disponível no Codex ou registre fallback documental.
