...
Todos os templates estão em PT-BR e foram adaptados do `.codex/templates/`.
...
A migração original de `.codex/templates/` foi realizada via `edit_file_available` pois o ambiente CMD estava instável (xcopy/robocopy travavam). Todos os 18 templates originais foram migrados e adaptados para PT-BR.
...
Os seguintes templates existem no `.codex/templates/` e devem ser copiados aqui quando necessário:
...
Atualize templates individualmente conforme a necessidade da story e valide com os quality gates do projeto.
