...
codex_runtime:
  activation: "Use esta skill local em .codex/skills/squad/SKILL.md"
  operational_root: ".codex/"
  shared_project_squads: "squads/"
  note: "No Codex, use leitura de arquivos, shell, web quando disponivel, apply_patch e comandos AIOX."

codex_tools:
  read: "rg, Get-Content, directory listing"
  edit: "apply_patch"
  shell: "PowerShell commands through Codex shell"
  research: "Codex browsing/search when available; otherwise ask for sources"
...
subagents:
  oalanicolas:
    ...
    location: '.codex/agents/oalanicolas.md'
  ...
  pedro-valerio:
    ...
    location: '.codex/agents/pedro-valerio.md'
...
1. Ler `.codex/agent-memory/squad/MEMORY.md` para contexto
...
Esta skill executa o workflow **`.codex/workflows/create-squad.md`**.
Para criação de mentes individuais: **`.codex/skills/clone-mind/SKILL.md`**.

## Adaptação Codex

- Criar squads de projeto em `squads/{squad-name}/`, mantendo isolamento self-contained.
- Usar `.codex/squads/` para squads globais versionados do runtime Codex.
- Usar `apply_patch` para editar arquivos e `rg`/shell para leitura e validação.
- Se a pesquisa web for necessaria, usar as capacidades de browsing/pesquisa disponiveis no Codex; se indisponiveis, registrar a limitacao antes de seguir.
- Nao depender de menus nativos ou hooks; executar por leitura local, `apply_patch` e comandos AIOX.
...
Todos os checks são **obrigatórios** (7/7 para PASS):

- [ ] SCOPE explícito (o que faz / o que não faz)
- [ ] Mínimo 5 heurísticas SE/ENTÃO
...
### SC_AGT_002: Fidelidade do Clone (agentes baseados em pessoas reais)

Todos os checks são **obrigatórios** (4/4 para PASS):
