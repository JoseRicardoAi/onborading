...
name: tech-search
description: |
  Self-contained deep tech research. Uses Codex browsing/search tools when available.
...
tool_hierarchy:
  ...
  deep_read:
    only: "available Codex page-reading/browsing capability"
    note: "No ETL, no shell, no external scripts. Use direct source reading with citations."

  execution_model:
    type: "concurrent-tools"
    engine: "Codex browsing/search tools when available; otherwise report limitation and ask for sources"
...
workflow:
  phases:
    ...
    1_auto_clarify:
      ...
      execution: |
        ...
        3. TECHNOLOGY DETECTION (case-insensitive):
           ...
           - AI/ML: LLM, RAG, LangChain, OpenAI, Anthropic, TensorFlow, PyTorch
    ...
    # ──────────────────────────────────────────────
    3_concurrent_search:
      name: "Concurrent Search & Fetch (Codex Runtime)"
      model_tier: "MAIN MODEL (Concurrent Tools)"
      description: |
        Dispatches all sub-queries as parallel tool calls in the same turn.
        Step A: Execute available Codex search/browsing for all queries.
        Step B: Parse URLs and read the best authoritative sources with citations.

      execution: |
        1. CONCURRENT SEARCH DISPATCH:
           Instead of spawning subagents, use the available Codex search/browsing
           capability for each of the 5-7 sub-queries. If browsing is unavailable,
           stop and report that external research requires user-provided sources.
        ...
        3. CONCURRENT FETCH DISPATCH:
           Read all selected authoritative URLs with the available Codex page-reading
           capability. Keep citations and source URLs in the final research artifacts.
      ...
      output: |
        {
           "search_results": [...],
           "tools_used": {"search": N, "page_read": N},
...
Query → Auto-Clarify → Decompose (ultrathink, MAIN MODEL)
              ...
              [Sub-query 1]  [Sub-query 2]  ... [Sub-query 5]
                   |              |                   |
            (Concurrent Codex search/browse invocations from MAIN MODEL)
                   |              |                   |
            (Collect URLs & concurrent Codex page reads)
                   |              |                   |
                   +------+-------+-------+-----------+
                          |
